...
public:
    ...
    // Blends a premultiplied-alpha overlay texture over the target ("over"
    // operator with SRC=ONE). Must be called between beginFrame and endFrame.
    void compositeOverlay(ID3D11ShaderResourceView* overlay, ID3D11RenderTargetView* target);
