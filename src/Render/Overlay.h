...
private:
    ...
    // Offscreen UI target: the menu is drawn into this cleared texture each
    // frame and composited over the back buffer, so no UI pixel ever survives
    // in the swap chain when the game skips a full redraw (cursor trails).
    DXGI_FORMAT m_format = DXGI_FORMAT_UNKNOWN;
    UINT m_compositeWidth = 0;
    UINT m_compositeHeight = 0;
    ID3D11Texture2D* m_overlayTexture = nullptr;
    ID3D11RenderTargetView* m_overlayTarget = nullptr;
    ID3D11ShaderResourceView* m_overlayView = nullptr;
