...
void ClickGui::updateSystemCursor(bool hide) {
    // ShowCursor keeps a per-queue display counter: the game re-shows the cursor
    // every time releaseMouse()/grabMouse() round-trips (holdMouseReleased fires
    // it repeatedly while the menu is open), so a single hide at open() is not
    // enough and the system cursor reappears next to the one we draw. Check the
    // real visibility each frame and only touch the counter when it changed.
    CURSORINFO info{};
    info.cbSize = sizeof(info);
    const bool visible = GetCursorInfo(&info) && (info.flags & CURSOR_SHOWING) != 0;
...
void ClickGui::renderChrome(const Layout& layout, float amount) {
    ...
    const std::string brand = [] {
        ...
        const auto* title =
            gui ? setting_cast<const TextSetting>(gui->findSetting("Title")) : nullptr;
        if (!title)
            return std::string{"anx1ous"};
        const auto first = title->value.find_first_not_of(" \t");
        if (first == std::string::npos)
            return std::string{"anx1ous"};
        const auto last = title->value.find_last_not_of(" \t");
        return title->value.substr(first, last - first + 1);
    }();

    const Vec2 title{layout.header.left + kPadding * 1.4f * scale,
                     layout.header.top + 17.0f * scale};
    DrawUtils::text(brand, title, theme.textActive.withAlpha(amount), 21.0f * scale,
                    Weight::SemiBold);
...
void ClickGui::renderConfigs(const Layout& layout, float amount) {
    ...
    const std::string placeholder =
        m_renaming ? "Rename '" + m_renameFrom + "'" : "New config name";
    const bool empty = m_nameBuffer.empty();
    const std::string shown = empty ? placeholder : m_nameBuffer;
    ...
    renderButton(createButton, m_renaming ? "Rename" : "Create", scale, theme.menuAccent());
    ...
    for (const std::string& name : names) {
        ...
        const Rect renameTo{remove.left - 8.0f * scale - small, buttonY, remove.left - 8.0f * scale,
                            buttonY + buttonHeight};
        const Rect saveTo{renameTo.left - 8.0f * scale - small, buttonY,
                          renameTo.left - 8.0f * scale, buttonY + buttonHeight};
        const Rect loadFrom{saveTo.left - 8.0f * scale - small, buttonY, saveTo.left - 8.0f * scale,
                            buttonY + buttonHeight};

        const std::string shownName =
            DrawUtils::fit(name, std::max(40.0f * scale, loadFrom.left - (card.left + 14.0f * scale)),
                           14.0f * scale);
        DrawUtils::text(shownName, {card.left + 14.0f * scale, nameY},
                        active ? theme.textActive : theme.text, 14.0f * scale,
                        active ? Weight::SemiBold : Weight::Medium);

        if (active) {
            const float width = DrawUtils::textWidth(shownName, 14.0f * scale, Weight::SemiBold);
        ...
        if (renderButton(renameTo, "Rename", scale, Colour::rgb(0x9AA3B4)))
            m_tooltip = "give this config a new name";
        ...
        m_hits.push_back(
            {renameTo, HitKind::ConfigRename, nullptr, nullptr, Category::Client, name});
...
void ClickGui::handleClick(const Vec2& cursor, bool right) {
    ...
    for (auto it = m_hits.rbegin(); it != m_hits.rend(); ++it) {
        ...
        case HitKind::ConfigCreate:
            if (m_renaming) {
                if (config.rename(m_renameFrom, m_nameBuffer)) {
                    modules::Notifications::push("Renamed to " + config.current(),
        ...
        case HitKind::ConfigRename:
            commitText();
            m_renaming = true;
            m_renameFrom = it->config;
            m_nameBuffer = it->config;
            m_editingName = true;
            keepEditing = true;
            input::InputManager::get().setCapture(true);
            break;
...
void ClickGui::onKey(KeyEvent& event) {
    ...
    if (m_editingName) {
        ...
        auto& config = Config::get();
        const std::string& buffer = m_nameBuffer;
        ...
        case VK_RETURN:
            if (m_renaming) {
                if (config.rename(m_renameFrom, buffer)) {
                    modules::Notifications::push("Renamed to " + config.current(),
                                                 modules::Notifications::Level::Success);
                    m_nameBuffer.clear();
                    m_renaming = false;
                    ...
                    modules::Notifications::push(
                        buffer.empty() ? "Enter a name first" : "That name is taken",
                        modules::Notifications::Level::Warning);
                }
                return;
            }
